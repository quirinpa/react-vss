...
export function getThemeMagic(themeName: string, getStyle: typeof makeThemeMagicBook) {
  const item = magicByTheme[themeName];

  if (item)
    return magic;

  const theme = getTheme(themeName);
  magicByTheme[themeName] = makeMagic(getStyle(theme, themeName), "." + themeName);
...
export function makeStyles(getStyle: typeof makeThemeMagicBook): () => Magic {
  return function () {
    return useMemo(() => magic = getThemeMagic(currentTheme, getStyle), [currentTheme]);
  };
}
...
export function useMagic(getStyle?: typeof makeThemeMagicBook) {
  const themeName = useTheme();
  return getThemeMagic(themeName, getStyle ?? makeThemeMagicBook);
}
